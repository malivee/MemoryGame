// Penjelasan file: DeckPuzzleScene.swift
// Alternative layout copied from GameScene: full-width board and fixed bottom deck.
...
final class DeckPuzzleScene: SKScene, UIGestureRecognizerDelegate {
    ...
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer is UISwipeGestureRecognizer { return point.y < viewport.minY }
    ...
    private func rotationTarget(at point: CGPoint) -> Int? {
        let candidates = tiles.filter { _, node in
            node.parent === canvas ? point.y < viewport.minY : viewport.contains(point)
        }
    ...
    @objc private func swipeDeck(_ gesture: UISwipeGestureRecognizer) {
        ...
        inventoryPage = min(pages - 1, max(0, inventoryPage + (gesture.direction == .left ? 1 : -1)))
        rebuild()
    }
