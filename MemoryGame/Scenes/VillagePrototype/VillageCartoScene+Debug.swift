...
extension VillageCartoScene {
    ...
    func showQuestDebugModal() {
        ...
        let panelH: CGFloat = min(size.height - 30, 330)
        ...
        // Header Title
        let title = SKLabelNode(text: "⚙️ DEBUG IN-GAME · SELEKTOR QUEST (1 - 7)")
        ...
        sub.position = CGPoint(x: 0, y: panelH / 2 - 38)
        ...
        let startY: CGFloat = panelH / 2 - 68

        // Kolom Kiri: Quest 1 - 4
        ...
        let rightQuests: [(title: String, desc: String, key: String, color: SKColor)] = [
            ...
            ("🌿 In-Game Q6: Perbukitan Herbal", "Daun perak & perjumpaan dengan The Hollow", "debug-quest-6", SKColor(red: 0.45, green: 0.92, blue: 0.65, alpha: 1)),
            ("🪵 In-Game Q7: Hutan & Jurnal Elias", "Cari 5 kayu bakar di lereng hutan & buku", "debug-quest-7", SKColor(red: 0.35, green: 0.75, blue: 0.48, alpha: 1)),
            ("⚡ In-Game Q7: Siap Buka Jurnal", "Kayu 5/5, langsung periksa tanah longsor", "debug-quest-7-step2", SKColor(red: 0.98, green: 0.55, blue: 0.32, alpha: 1))
        ]
        ...
        let bottomY = -panelH / 2 + 25
        let bottomBtnW = (panelW - 60) / 2.0

        // Tombol Reset Semua
        let resetBtn = SKShapeNode(rectOf: CGSize(width: bottomBtnW, height: 30), cornerRadius: 8)
        ...
        let finishAllBtn = SKShapeNode(rectOf: CGSize(width: bottomBtnW, height: 30), cornerRadius: 8)
        ...
        let finishAllLbl = SKLabelNode(text: "🏆 Selesaikan Semua Quest (1-7)")
        finishAllLbl.fontName = "AvenirNext-Bold"
        finishAllLbl.fontSize = 10.5
    ...
    func warpToQuest(stage: Int, step2: Bool = false) {
        dismissQuestDebugModal()
        stopInput()
        selected = nil
    ...
    func completeAllQuests() {
        ...
        setArthurSafePosition(layout.world(sourcePosition) ?? VillageTileLayout.initial.center)
        save()
        rebuild("Debug In-Game: Seluruh Quest 1 - 7 selesai! Gudang Kosong (Markas Rahasia) telah terbuka.")
    }
    ...
    func resetAllQuests() {
        ...
        setArthurSafePosition(layout.world(sourcePosition) ?? VillageTileLayout.initial.center)
        save()
        rebuild("Debug In-Game: Seluruh progres Quest 1 - 7 telah di-reset ke awal mula.")
    }
}
